...
public class RedisChatMemoryRepository implements ChatMemoryRepository {
    ...
    public List<Message> findByConversationId(String conversationId) {
        ...
        // 获取列表中所有消息
        List<String> jsonMessages = listOps.range(0, -1);
        if (null == jsonMessages || jsonMessages.isEmpty()) {
            return List.of();
        }
...
//        messages.forEach(message -> listOps.rightPush(JSONUtil.toJsonStr(message)));//JSONUtil有序列化问题
        // 将消息序列化并添加到Redis列表的右侧
        messages.forEach(message -> listOps.rightPush(MessageUtil.toJson(message))); //使用自定义的序列化方法
    }
