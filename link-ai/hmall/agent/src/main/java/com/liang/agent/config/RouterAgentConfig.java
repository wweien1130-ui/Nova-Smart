    ...
    public ChatClient routerChatClient(ChatModel openapiChatModel) {
        return ChatClient.builder(openapiChatModel)
                .defaultSystem("""
                        ...
                        - ORDER: 订单管理相关的问题（创建、查询、取消、确认收货）
                        - ADDRESS: 地址管理相关的问题（查询、添加、更新、删除地址）

                        路由规则：
                        1. 如果用户的问题涉及"搜索商品"、"查找商品"、"推荐商品"、"有什么商品"等，路由到 ITEM
                        2. 如果用户的问题涉及"加购"、"加入购物车"、"添加到购物车"、"买这个"、"下单这个"、"要了"、"就要这个"等确认购买意图，路由到 CART
                        3. 如果用户的问题涉及"查看购物车"、"购物车有什么"、"移除商品"等购物车管理，路由到 CART
                        4. 如果用户的问题涉及"下单"、"创建订单"、"查看订单"等，路由到 ORDER
                        5. 如果用户的问题涉及"地址"、"收货地址"等，路由到 ADDRESS
                        6. 如果用户只是说"对"、"好的"、"可以"等确认语，结合对话历史判断意图：
                           - 如果上一轮是商品搜索，用户确认后应路由到 CART（表示要加购）
                           - 如果上一轮是其他操作，根据上下文判断

                        请严格按照以下 JSON 格式返回结果，不要返回其他内容：
                        {"agentType": "ITEM", "question": "用户原始问题"}
                        """)
